...
describe("模块图", () => {
  ...
  it("大文件只许变小", () => {
    ...
    const CEILINGS: Record<string, number> = {
      ...
      // 而那种 bug 在截图和测试里都看不见。
      // 2558 → 2693：补上 blobShadows（贴地圆斑）。它和上面那条阴影缓存是**一对**，
      // 上一版只移植了缓存、把圆斑当成可选的第二笔 —— 那是错的：关掉逐帧重画之后，
      // 会动的东西（人、狼、猎物）的真阴影会冻在上一次重画的位置，实机表现就是
      // 「影子延时才跟上人」。这段注释必须留着，否则下一个人还会把这两件事拆开。
      "src/render/GameRenderer.ts": 2693,
      ...
      // 450 → 466：狼与猎物改推贴地圆斑，并在入场时移出阴影图。理由同 GameRenderer。
      "src/render/entities/CreatureViews.ts": 466,
