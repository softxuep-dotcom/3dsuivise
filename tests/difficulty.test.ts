...
describe("难度 · 五轴与狼群压力", () => {
  ...
  it("前三夜攻营配额只做小幅阶梯，而不是近乎翻倍", () => {
    // 基数 EARLY_NIGHT_RAID_TARGETS = [5, 9, 14]，各难度乘 tuning.raid 后四舍五入：
    //   easy   ×1.0 → 5 / 9 / 14
    //   normal ×1.2 → 6 / 11 / 17
    //   insane ×1.4 → 7 / 13 / 20
    // 第一夜 2026-08-24 从 4 改回 5：降到 4 的前提是夜长缩到 100 秒，
    // 而夜长已经改回 150，见 WolfDirector 里 EARLY_NIGHT_RAID_TARGETS 那段。
    // 这条测试钉的是"阶梯要小幅"，不是这几个数本身 —— 改配额时同步更新即可。
