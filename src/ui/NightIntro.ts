 ...
 * ## 三拍
 *
 *   1. 停 —— 时间冻住，一声狼嚎，镜头从人物推到营火上（2.2 秒，不需要操作）
    ...
    this.beats = [
      {
         ...
         * 3.4 → 2.2 秒。
         *
         * 这是全场唯一一段**玩家什么都做不了**的时间：世界冻着、画面压暗、
         * 镜头自己在走。而 updateCamera 里推镜的插值是 1.35 秒（回程 1.0 秒），
         * 也就是说 3.4 秒里有整整 2 秒是镜头到位之后的干等 —— 那 2 秒不传递任何信息，
         * 只是在消耗一个刚入夜、正紧张的玩家的耐心。
         *
         * 2.2 = 1.35（推镜走完）+ 0.85（够看清镜头停在营火上）。再短就会在镜头
         * 还没到位时切走，那一推就白推了 —— 这一拍要说的"火塘是今晚的中心"
         * 全靠镜头的落点，不靠文案。
         *
         * 后面两拍不算在"干等"里：它们是交互拍，玩家一动就推进（最短 0.9 / 1.2 秒）。
         */
        minSeconds: 2.2,
        timeoutSeconds: () => 2.2,
      },
