 ...
 *
 * 第一夜 4 → 5：跟着 FIRST_NIGHT_DURATION 一起改回来（2026-08-24）。
 *
 * 5→4 那一刀是**因为**夜长从 150 缩到 100 才切的 —— 配额和夜长解耦，
 * 缩夜长只缩跑道、不减车流，密度会凭空涨 50%：
 *
 *     150s / 5 只 = 每只摊 30 秒   ← 现在（和 1.0.28 一致）
 *     100s / 5 只 = 每只摊 20 秒     密度 +50%，所以当时要降配额
 *     100s / 4 只 = 每只摊 25 秒     曾经
 *
 * 夜长改回 150 之后这条的前提就没了。留着 4 会变成 150s / 4 只 = 每只摊 37.5 秒，
 * **比 1.0.28 还稀** —— 那是一次没人要过的降难度，而且它会污染参照点。
 *
 * 实测两者没有可测差别（autoplay 20 局：配额 5 平均 3m48s、配额 4 平均 3m50s，
 * 活过第一夜都是 55%）。改回 5 不是因为 5 更好，是为了让"1.0.28 的夜间配置"
 * 重新成为一个干净的基线。
 *
 * 巡逻野狗不受影响：它们按 spawnInterval 在夜晚前 45 秒刷完，和夜长无关。
 */
 ...
 *
 * 仇恨半径原来是 14 米，从布局倒推：三桶油就在守卫脚下，所以去拿桶必定惊动它们；
 * 而卡车停在 30 米外，所以走到车边、装车、发车全程都不会拉到仇恨。
 *
 * **14 米有个当时没算的上界：它比画面还大。** 等距视角的可见范围极不对称，
 * 朝镜头那一侧最短 —— 实测竖屏 375×812 只有 9.4 米
 * （见 GameRenderer.updateCamera 那张表）。也就是说站在守卫的下坡方向时，
 * 它在**屏幕外**就已经醒了，玩家看到的是一只从画面边缘冲进来的狗，
 * 没有任何可以据此做判断的前兆。这不是难度，是信息缺失。
 *
 * 收到 8 米，取的就是那个下界：**8 < 9.4，醒的那一刻它一定在画面里**，余 1.4 米。
 * 这个不等式**吃相机系数**：把镜头拉近到 0.98/0.58 时下限只剩 8.5 米（余 0.5 米），
 * 再近就会破。要动 PORTRAIT/LANDSCAPE_CAMERA_SCALE，先回来核这一条。
 * 下界另一头也验过：五只守卫摆在半径 4.8 米的环上（见 seedDenGuards），
 * 环内任意一点到最近守卫最远 3.0 米，所以"拿桶必定惊动"这条照旧成立，
 * 而 8 米离卡车那 30 米还差得远，"先去看车再决定打不打"也照旧。
 *
 * GUARD_LEASH 没跟着动。它管的是"追出去多远就折回"，配 8 米的仇恨，
 * 玩家仍有 14 米的空间把一只守卫拉离狗群单杀 —— 按比例缩到 13 米的话
 * 只剩 5 米，这个战术就等于没了。要收的是"什么时候醒"，不是"能不能拉"。
 */
const DEN_GUARD_COUNT = 5;

const GUARD_AGGRO_RADIUS = 8;
...
export class WolfDirector {
  ...
  spawnWolf(options: { role?: WolfRole; forceKind?: WolfKind; origin?: Vec2 } = {}): void {
    ...
    if (kind === "elite") {
      ...
      /*
       * 防御 7 → 3（docs/一局节奏表.md §2 的清单第 6 条）。
       *
       * 伤害是**减法**：max(1, round(raw) - defense)。防 7 对初始匕首（攻 30）
       * 砍掉 23%，对三阶（攻 55）只砍 13% —— 减法结构额外惩罚的正是还没升级的人，
       * 而 92% 的玩家活不到能升级的那一夜。
       *
       * 同时这一改让一阶不再"白做"：
       *   防 7：匕首 23 实伤 → 7 刀   sword-1(38) 31 实伤 → 6 刀   差 1 刀
       *   防 3：匕首 27 实伤 → 6 刀   sword-1(38) 35 实伤 → 5 刀   差 1 刀，但基线短了一刀
       * 砍死一只大狼从 7 刀降到 6 刀，交火时间短了 14%，挨咬次数跟着降。
       *
       * 只动防御、不动 160 血和 20 咬伤：大狼作为"装备门槛"的定位没变，
       * 变的是这道门槛不再对新手额外加价。
       */
      defense = 3;
  ...
  private wolfCanSeePlayer(wolf: WolfState): boolean {
    ...
    // 半径收到 8 米作为交换 —— 比别的狼（14.5）看得近不少，但看得全。
