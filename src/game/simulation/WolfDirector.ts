 ...
 *
 * 第一夜 5 → 4：夜长刚从 150 秒缩到 100 秒（见 FIRST_NIGHT_DURATION），
 * 而这个配额**和夜长完全解耦** —— 缩夜长只缩跑道，不减车流：
 *
 *     150s / 5 只 = 每只摊 30 秒     波次中心 22.5s / 67.5s
 *     100s / 5 只 = 每只摊 20 秒     波次中心 15.0s / 45.0s   ← 密度涨了 50%
 *     100s / 4 只 = 每只摊 25 秒     波次中心 15.0s / 45.0s   ← 现在
 *
 * 取 4 而不是 3，是为了保住两波的节奏：waveCount = round(配额 / 2.5)，
 * 配额 3 会塌成一波，"一波打完能喘口气"就没了，还可能碰到
 * "风脊营地至少 2 只摸到玩家"那条回归。4 是 2 波 × 2 只。
 *
 * 巡逻野狗不受影响：它们按 spawnInterval 在夜晚前 45 秒刷完，和夜长无关。
 */
const EARLY_NIGHT_RAID_TARGETS = [4, 9, 14] as const;
