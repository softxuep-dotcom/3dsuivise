 ...
 * 装备表：武器 / 护甲的分线、造价与属性。
 *
 * 从 GameSimulation.ts 拆出来的一整块**数据** —— 它没有任何行为，只回答"这一件
 * 装备是什么"。放在一起是因为四条线（刀 / 剑 / 铁甲 / 皮甲）的平衡是**互相定义**
 * 的：改动任何一格都得对着另外三条线读，散在三千行实现里只会读错。
 */
import type { ArmorKind, EquipLine, InventoryItemKind, WeaponKind } from "./types";
