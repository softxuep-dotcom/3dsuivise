...
import type { CritterKind, CritterState, InventoryItemKind, LocalizedText, Vec2 } from "./types";
import {
...
import { distance, distanceSquared, direction, normalize, rotateToward, TAU } from "./geometry";
...
/**
 * 猎物：拾骨鸦、沙兔、长角羚。**它们不追人、不攻击，只是活着然后被吃掉。**
 *
 * 是 WolfDirector 的小号版本，端口和结构照它抄，但简单得多 —— 猎物没有阵营、
 * 没有夜袭、没有撤退，只有"晃悠"和"跑"两个模式。
 *
 * 它承担的玩法职责其实不小：**肉和皮的唯一来源**（长角羚还是唯一会掉水的猎物），
 * 以及开局那三只教学猎物 —— 玩家第一次挥刀命中的时间从 43.2 秒压到 0.6 秒，
 * 全靠它们撒在脚边（见 {@link CritterDirector.tutorialPreySpot}）。
 */
export interface CritterOwner extends SimContext {
  /**
   * 出生点与初始朝向。教学猎物要撒在**这里**，不能拿 `player` 当锚点：
   * seedCritters 是等模型下载完才跑的，那时玩家早就走开了。
   */
  readonly spawnAnchor: Vec2;
  readonly spawnFacing: number;
  /** 猎物死了掉东西。掉落物归世界管，不归猎物管。 */
...
export class CritterDirector {
  ...
  constructor(
    private readonly owner: CritterOwner,
  ...
  update(delta: number): void {
    ...
    for (const critter of this.critters) {
      if (distanceSquared(critter, this.owner.player) > lodCutoff) {
   ...
   *
   * 角度和半径都是写死的常量，不走 this.owner.random()：一是这三只本来就该稳定出现在
  ...
  private tutorialPreySpot(index: number): Vec2 {
    ...
    const angle = this.owner.spawnFacing + spread;
    return {
      x: this.owner.spawnAnchor.x + Math.cos(angle) * radius,
      z: this.owner.spawnAnchor.z + Math.sin(angle) * radius,
    };
  }
  ...
  private findCritterSpawnPoint(minPlayerDistance = 30): Vec2 | null {
    for (let guard = 0; guard < 24; guard += 1) {
      const point = {
        x: (this.owner.random() - 0.5) * (this.owner.world.size - 20),
        z: (this.owner.random() - 0.5) * (this.owner.world.size - 20),
      };
      // 别在玩家眼皮底下凭空出现。
      if (distanceSquared(point, this.owner.player) < minPlayerDistance * minPlayerDistance) continue;
      if (!isTerrainWalkable(this.owner.world, point)) continue;
      return point;
    }
    return null;
  }
  ...
  private spawnCritter(kind: CritterKind, origin: Vec2): void {
    const spec = CRITTER_SPECS[kind];
    const spawn = this.collision.findNearestWalkablePoint(origin);
    const facingAngle = this.owner.random() * TAU;
    this.critters.push({
      id: this.critterId++,
      kind,
      ...spawn,
      ...
      mode: "graze",
      anchor: { ...spawn },
      wanderAngle: this.owner.random() * TAU,
  ...
  private updateCritter(critter: CritterState, delta: number): void {
    ...
    const playerDistance = distance(critter, this.owner.player);
    ...
    if (critter.mode === "flee") {
      desired = direction(this.owner.player, critter);
      pace = spec.fleeSpeed;
    } else {
      // 游荡：绕着锚点慢慢晃，离得太远就往回收。
    ...
    // 但快不代表甩得掉。
    const steered = this.collision.getSteeredDirection(critter, desired);
    critter.facing = rotateToward(critter.facing, steered, spec.turnRate * delta);
    this.collision.moveEntity(critter, critter.facing.x * pace * delta, critter.facing.z * pace * delta, 0.4, false);
  }
  ...
  kill(critter: CritterState): void {
    ...
    if (spec.meat > 0) this.owner.createDrop(critter, "raw-meat", -0.6, spec.meat);
    if (spec.hide > 0) this.owner.createDrop(critter, "hide", 0.6, spec.hide);
    // 长角羚是唯一会掉水的猎物：沙漠里猎杀大型有蹄类取体液是真实做法。
    if (spec.water > 0) this.owner.createDrop(critter, "water", 1.8, spec.water);
    this.owner.emit({ type: "critter-killed", critterId: critter.id, kind: critter.kind });
  }

  label(kind: CritterKind): LocalizedText {
