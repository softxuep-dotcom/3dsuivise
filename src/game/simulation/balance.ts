 ...
 *
 * autoplay 那边同一个方向：**活过第一夜不随夜长变化**，而且这一条两套机器人都同意 ——
 * 旧策略（会被石头废掉武器，见 tools/autoplay.ts 的 usesThrow）读 55% / 55%，
 * 修好之后读 100% / 100%。平均单局 5m51s（夜100）对 5m58s（夜150），7 秒，噪声。
 *
 * 当初那张"活到黎明"的表为什么会骗人：**黎明会跟着夜长一起提前**。终点自己往回挪，
 * 存活率当然涨 —— 量的其实是"跑道变短了"，不是"人变强了"。要量第一夜难不难，
 * 得量死亡时刻或贴身次数，不能量一个终点会跟着改的指标。
 *
 * 有一句要更正：本地一度读出「回退值 +10~14 秒」，那是用坏掉的机器人量的
 * （它捡起野外石头就再也不挥刀，于是配置里石头多的那一版被系统性压低）。
 * 换成修好的机器人重量，差值缩到 7 秒 —— 本地这半边其实是**没差别**，
 * 而不是「回退更好」。所以这条回退真正的依据是 Fit Test 那半边：
 * 时段校正之后 1.0.28 / 1.0.29 / 1.0.30 全部落在 172~176 秒，
 * 缩夜长**什么也没买到**，而它是个改动，改动就该退回去。
 */
 ...
 * 走到几米内可以捡起地上的枯木或石头。
 *
 * 原本是三处写死的 2.5（requestInteraction、hasNearerTarget、getInteractionHint），
 * 三处必须相等 —— 否则会出现"提示说能捡、按下去没反应"，或者反过来，
 * 拾取抢走了本该给火塘的那一下。提成常量是为了让它们没法再分家。
 */
export const ITEM_PICKUP_REACH = 2.5;
