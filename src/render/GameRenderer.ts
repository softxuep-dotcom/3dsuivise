...
import { terrainHeightAt } from "../game/terrain/TerrainModel";
...
import type { SceneryContext } from "./scenery";
import {
  buildCampWalls,
  buildDens,
  buildGround,
...
export class GameRenderer {
  ...
  constructor(
    ...
    this.terrainMesh = buildGround(this.scenery, this.renderer);
    buildCampWalls(this.scenery);
    this.buildTrees();
    buildGroundCover(this.scenery);
    buildLandmarks(this.scenery);
    ...
    this.blobShadows = this.lowPower ? createBlobShadows() : null;
    if (this.blobShadows) this.scene.add(this.blobShadows);
    buildDens(this.scenery);
    this.buildCamps();
    this.buildCacti();
    this.buildIronNodes();
    this.buildWells();
  ...
  private buildPlayer(): {
    ...
    for (const [kind, spec] of Object.entries(WEAPON_VISUALS) as Array<[WeaponKind, BladeVisual]>) {
      const view = createBladeView(spec, this.lowPower);
    ...
    const carriedStake = createStakeView();
  ...
  /**
   * 交给 scenery.ts 的那一小片世界。每次现取而不是缓存 —— `world` 在 resetRun 里会换。
   */
  private get scenery(): SceneryContext {
    return {
      scene: this.scene,
      world: this.world,
      lowPower: this.lowPower,
      worldHeight: (x, z) => this.worldHeight(x, z),
    };
  }
