 ...
 * ## 一档为什么分设备，二三档为什么不分
 *
 * 一档是**起点**，得贴着设备本来的能力，而这两个数都是现状：桌面一直是 1.6，
 * 移动一直是 1.0，加梯子不该顺手改动任何一边的默认表现。
 *
 * 二三档是**终点**，问的是"这台机器还剩多少"，那跟它是手机还是电脑没关系了。
 * 一台跟不上的弱笔记本和一台跟不上的手机，需要的是同一个数。
 *
 * ## 手机上一档降到二档，pixelRatio 是不动的
 *
 * 因为两边都是 1.0 —— 手机的一档本来就已经压到底了。二档对手机的意义全在
 * **关雾和停扬沙**，那两样都是每片元的开销，正好是填充率吃紧时最该先砍的。
 * 手机真正的 pixelRatio 下调只发生在进三档那一步（1.0 → 0.8）。
 *
 * 桌面则相反：一档到二档是 1.6 → 1.0，那一下就把像素数砍掉六成。
 *
 * 全部还要过 min(devicePixelRatio, …)：dpr 1.0 的普通显示器本来就到不了 1.6。
 */
