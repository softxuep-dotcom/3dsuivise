 ...
 *
 *   一档（默认）  pixelRatio 移动 1.0 / 桌面 1.6   有雾  有扬沙  剔除 45 米
 *   二档          pixelRatio 1.0                  无雾  无扬沙  剔除 45 米
 *   三档          pixelRatio 0.8                  无雾  无扬沙  剔除 35 米
 *
 * ## 为什么第一次判定的窗口比后面短
 *
 * 一档就是改造前的默认表现（桌面 1.6、移动 1.0），所以顶着一档跑**不存在
 * 额外风险** —— 这一条跟"赶紧降下来"无关。短窗口要的是另一件事：
 * 真正跟不上的机器早一秒进二档，就早一秒好玩，而降档本身对玩家近乎无感
 * （关雾、停扬沙）。开局那几十秒又恰好最要紧（731 局里 36% 死在第一个白天的
 * 40 秒内），值得先给一个粗判。
 *
 * 后面的轮次用完整窗口：那时已经不赶时间了，样本多一些、判得稳一些更划算。
 *
 * ## 为什么慢得离谱时一次降两档
 *
